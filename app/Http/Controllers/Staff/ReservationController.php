...
class ReservationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // If calendar view key is present or we are fetching all for a range, we might want to skip pagination
        // But for safety let's keep pagination high or standard.
        // If 'all' is requested for calendar:
        if ($request->filled('get_all')) {
            $reservations = $query->orderBy('start_time', 'asc')->get(); // No pagination for calendar logic if manageable
        } else {
            $reservations = $query->orderBy('start_time', 'desc')->paginate(20)->withQueryString();
        }
