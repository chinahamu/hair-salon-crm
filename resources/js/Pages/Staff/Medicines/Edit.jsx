...
export default function Edit({ auth, medicine }) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        name: medicine.name || '',
        description: medicine.description || '',
        unit: medicine.unit || '',
        alert_threshold: medicine.alert_threshold || 0,
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('staff.medicines.update', medicine.id));
    };

    const handleDelete = () => {
        if (confirm('本当にこの薬剤を削除しますか？')) {
            destroy(route('staff.medicines.destroy', medicine.id));
        }
    };
